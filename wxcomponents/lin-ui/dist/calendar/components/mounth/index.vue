<template>
<uni-shadow-root class="lin-ui-dist-calendar-components-mounth-index"><view class="calendar-mounth-container">
  <view class="title" v-if="showMonthTitle">
    {{ computed.formatMonthTitle(date) }}
  </view>
  <view class="calendar-day-container">
    <view v-for="(item,index) in (days)" :key="item.index" class="calendar-day-wrap">
      <day @click.native="onClick" :data-item="item" v-if="item.type !== 'empty'" :text="item.text" :topInfo="item.topInfo" :bottomInfo="item.bottomInfo" :type="item.type" :color="' '+(color)"></day>
    </view>
  </view>
...
global['__wxVueOptions'] = {components:{'day': Day}}

global['__wxRoute'] = 'lin-ui/dist/calendar/components/mounth/index'
...
export default global['__wxComponents']['lin-ui/dist/calendar/components/mounth/index']
</script>
