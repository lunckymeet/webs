<template>
<uni-shadow-root class="lin-ui-dist-loading-index"><view :class="'container-loading l-container-class '+(fullScreen ? 'content': '')" v-if="show && fullScreen" :style="'background:'+(bgColor)+';opacity:'+(opacity)+';z-index:'+(zIndex)" @touchmove.stop.prevent="doNothingMove">
  <view v-if="show" :class="'l-class '+(type + '-spinner')+' '+(type==='change'||custom?'':'spinner-' + type + '-' + size)" :style="'margin-bottom:'+(distance)+'px'">
    <block v-if="custom">
      <slot></slot>
    </block>
    <block v-else>
      <view v-if="type==='flash' || type==='change' || type==='flip'" :style="color?'background-color:'+color:''" :class="(type+'-bounce1')+' '+('spinner-'+ type + '-' + size)"></view>
      <view v-if="type==='flash' || type==='change'" :style="color?'background-color:'+color:''" :class="(type+'-bounce2')+' '+('spinner-'+ type + '-' + size)"></view>
      <view v-if="type==='change'" :style="color?'background-color:'+color:''" :class="(type+'-bounce3')+' '+('spinner-'+ type + '-' + size)"></view>
      <view :class="'spinner-circle '+('spinner-circle-' + size)" v-if="type === 'circle'">
      ...
      <view v-if="type === 'rotate'">
        <view :class="'rotate rotate-'+(size)" :style="'border-color: '+(color)+';'"></view>
      </view>
    </block>
  </view>
</view>
<view v-if="(!fullScreen)" class="l-container-class" style="position: relative">
    <slot name="content"></slot>
    <view v-if="show" class="inner-loading-container" :style="'background:'+(bgColor)+';opacity:'+(opacity)+';z-index:'+(zIndex)"></view>
    <view v-if="show" class="l-class loading-icon-container" :style="'z-index:'+(zIndex + 1)">
        <view :class="(type + '-spinner')+' '+(type==='change'||custom?'':'spinner-' + type + '-' + size)">
            ...
            <block v-else>
                ...
                <view v-if="type==='flash' || type==='change'" :style="color?'background-color:'+color:''" :class="(type+'-bounce2')+' '+('spinner-'+ type + '-' + size)"></view>
                <view v-if="type==='change'" :style="color?'background-color:'+color:''" :class="(type+'-bounce3')+' '+('spinner-'+ type + '-' + size)"></view>
                <view v-if="type === 'circle'" :class="'spinner-circle '+('spinner-circle-' + size)">
                    <view class="spinner-container container1">
                        <view :class="'circle1 container-view '+(type + '-' + size)" :style="'background: '+(color)"></view>
                        <view :class="'circle2 container-view '+(type + '-' + size)" :style="'background: '+(color)"></view>
                        <view :class="'circle3 container-view '+(type + '-' + size)" :style="'background: '+(color)"></view>
...
global['__wxRoute'] = 'lin-ui/dist/loading/index'
...
export default global['__wxComponents']['lin-ui/dist/loading/index']
</script>
