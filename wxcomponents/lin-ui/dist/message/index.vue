<template>
<uni-shadow-root class="lin-ui-dist-message-index"><view :class="'l-message l-class '+('l-message-'+type)+' '+(status?'l-message-show':'')" :style="'z-index:'+(zIndex)+';top:'+(top)+'rpx'">
  <block v-if="status">
    <view style="margin-right:15rpx">
      <l-icon :name="icon?icon:type" :size="iconSize" :color="type==='warning'?'#333':iconColor"></l-icon>
    </view>
    <image v-if="image" :src="image" class="l-message-image l-class-image l-image-class"></image>
    {{content}}
    <slot></slot>
  </block>
...
global['__wxRoute'] = 'lin-ui/dist/message/index'
...
export default global['__wxComponents']['lin-ui/dist/message/index']
</script>
