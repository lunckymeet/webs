<template>
<uni-shadow-root class="lin-ui-dist-spin-index"><view v-if="show" :class="(type==='flash'?'l-class':'')+' '+(type + '-spinner')+' '+(type==='change'||custom?'':'spinner-' + type + '-' + size)">
  <block v-if="custom">
    <slot></slot>
  </block>
  <block v-else>
    <view :style="color?'background-color:'+color:''" :class="'l-class '+(type+'-bounce1')+' '+('spinner-'+ type + '-' + size)"></view>
    <view v-if="type==='flash' || type==='change'" :style="color?'background-color:'+color:''" :class="'l-class '+(type+'-bounce2')+' '+('spinner-'+ type + '-' + size)"></view>
    <view v-if="type==='change'" :style="color?'background-color:'+color:''" :class="'l-class '+(type+'-bounce3')+' '+('spinner-'+ type + '-' + size)"></view>
  </block>
...
global['__wxRoute'] = 'lin-ui/dist/spin/index'
...
export default global['__wxComponents']['lin-ui/dist/spin/index']
</script>
